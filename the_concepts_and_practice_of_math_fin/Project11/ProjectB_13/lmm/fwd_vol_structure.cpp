...
std::vector<std::vector<double>> FwdVolStructure1::integral(const std::vector<double>& time, double t) const {
	...
	std::vector<std::vector<double>> I_t(time.size(), std::vector<double>(time.size(),0.0));
		
	for (size_t i(0); i < time.size(); ++i){
		for (size_t j(0); j <time.size(); ++j){

			double tau_i(t - time[i]); //tau_i*-1.0;
			double tau_j(t - time[j]); //tau_j*-1.0;
...
//available at http://www.cs.utah.edu/~cxiong/
	...
	std::vector<std::vector<double>> I_t = this->integral(time, t); 
	std::vector<std::vector<double>> I_T = this->integral(time, T);
...
FwdVolStructureFlat::FwdVolStructureFlat(double vol_, double beta_)
	:vol(vol_), FwdVolStructure(beta)
{}

void FwdVolStructureFlat::bumpVol(double param) { vol += param; }
...
void FwdVolStructureFlat::covariance(matrix<double>& cov_matrix,
	...
	for (size_t i(0); i < cov_matrix.size1(); ++i)
		for (size_t j(0); j < cov_matrix.size2(); ++j)
			cov_matrix(i,j) =	this->correlation(time[i], time[j]) * K[i] * K[j] * vol * vol;
