...
namespace {
	...
	struct CommonVars{
		...
		void makeIndex(std::vector<Date> dates, std::vector<Rate> rates) {
			...
			process = boost::shared_ptr<LiborForwardModelProcess>(new LiborForwardModelProcess(size + 2, index));
		}

		boost::shared_ptr<LiborForwardModel> makeModel(Real a, Real b, Real c, Real d, Real correl){

			boost::shared_ptr<LmCorrelationModel> corrModel(new LmExponentialCorrelationModel(size + 2, correl));
		...
		boost::shared_ptr<VanillaSwap> makeSwap(Rate strike) {
		...
		CommonVars(Size start_,Size length){
	...
	)
{
	...
	CommonVars vars(start,length);

	std::vector<Date> dates;
	std::vector<Rate> rates;

	dates.push_back(todaysDate_); 
	dates.push_back(todaysDate_ + (start + length) * Years + 1 * Months);
	...
	try {
		...
		boost::shared_ptr<VanillaSwap> swap_ = vars.makeSwap(strike);

		//std::cout << swap_->fairRate() << std::endl;
