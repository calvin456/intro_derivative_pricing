                                    ...
                                    TheGenerator(TheGenerator_)
{
	...
	Drifts[0] = R_.Integral(0.0, Times[0]) - D_.Integral(0.0, Times[0]) * Spot;
	StandardDeviations[0] = sqrt(Variance) * Spot;

    for (unsigned long j=1; j < NumberOfTimes; ++j)
    {
		double thisVariance = Vol_.IntegralSquare(Times[j - 1], Times[j]);
		Drifts[j] = R_.Integral(Times[j - 1], Times[j]) - D_.Integral(Times[j - 1], Times[j])* Spot;
		StandardDeviations[j] = sqrt(thisVariance)* Spot;
    }
